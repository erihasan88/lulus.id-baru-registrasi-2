        ...
        if (reg) {
          setStudents(prev => {
            const alreadyExists = prev.some(s =>
              s.username === reg.username || s.nisn === reg.biodata?.nisn
            );
            ...
            return [...prev, {
              ...
              kk: reg.biodata?.kk || '',
              jk: reg.biodata?.jk || 'Laki-laki',
              tempatLahir: reg.biodata?.tempat_lahir || '',
              tglLahir: reg.biodata?.tgl_lahir || '',
              program: reg.program_paket || 'Paket C',
              kelas: reg.biodata?.rombel_nama || 'Belum Ditentukan',
              tipeKelas: reg.tipe_kelas || 'Reguler',
              tahunAjaran: '2026/2027',
              alamat: reg.biodata?.alamat || '',
              ibu: reg.biodata?.nama_ibu || '',
              ayah: reg.biodata?.nama_ayah || '',
              status: 'Aktif',
              dokumen: reg.dokumen || {
                foto: '',
                ktp: '',
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {Object.entries(selectedReg.dokumen).map(([key, val]: any) => {
                    console.log("DOKUMEN PREVIEW:", key, val);
                    return (
