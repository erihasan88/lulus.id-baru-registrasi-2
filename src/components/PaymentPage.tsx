...
export const PaymentPage: React.FC<PaymentPageProps> = ({
  ...
  const fetchInvoice = async () => {
    try {
      ...
      const savedSettings = localStorage.getItem('lulus_admin_payment_settings');
      if (savedSettings) {
        try {
          setAdminPaymentSettings(JSON.parse(savedSettings));
        } catch (e) {
          console.log('Gagal membaca pengaturan pembayaran admin');
        }
      }
  ...
  const handleUploadProof = async () => {
    ...
    setIsUploading(true);
    try {
      await api.uploadPaymentProof(proofFile, paymentMethods.find(m => m.id === selectedMethod)?.name || 'Transfer');
  ...
  if (loading) {
    return (
      <div className="min-h-screen overflow-y-auto bg-background-aplikasi text-slate-100 flex flex-col font-sans">
...
console.log("PAYMENT DEBUG", {
  ...
  return (
    <div className="min-h-screen bg-background-aplikasi text-slate-100 flex flex-col font-sans">
      {/* Top Header Banner */}
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card-aplikasi border border-slate-800 rounded-3xl p-6 shadow-xl space-y-6 relative overflow-hidden">
            ...
            <div className="p-4 bg-background-aplikasi border border-slate-850 rounded-2xl flex items-center justify-between">
              <div>
              ...
              <button 
                ...
                className="px-3.5 py-2 bg-card-aplikasi hover:bg-slate-800 text-[10px] font-black text-slate-300 rounded-xl border border-slate-800 transition-all flex items-center gap-1.5 cursor-pointer"
              >
          ...
          {!isPaid && !isWaitingConfirm && (
           <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl space-y-6">
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left flex flex-col justify-between transition-all cursor-pointer ${
                      selectedMethod === method.id
                        ? 'bg-blue-500/10 border-pink-500 text-blue-400'
                        : 'bg-background-aplikasi border-slate-850 hover:border-slate-800 text-slate-300'
                    }`}
                  >
                    {method.type === 'QR' ? (
                    ...
                    <div>
                      ...
                      <span className="block text-[8px] text-slate-500 font-bold mt-0.5 uppercase tracking-wide">
                        {method.type === 'QR' ? 'QRIS Digital Payment' : 'Virtual Account / Transfer'}
                      </span>
                    </div>
                  </button>
                ))}
              ...
              <div className="bg-background-aplikasi p-5 rounded-2xl border border-slate-850 space-y-4">
                {paymentMethods.find(m => m.id === selectedMethod)?.type === 'QR' ? (
                  <div className="flex flex-col md:flex-row items-center gap-5">
                    <div className="p-3.5 bg-white rounded-2xl shrink-0">
                      {/* Generated live QR code mock placeholder */}
                      <img 
                        src={adminPaymentSettings?.qrisUrl || paymentMethods.find(m => m.type === "QR")?.value} 
                        alt="QRIS QR Code" 
                  ...
                  <div className="space-y-3 text-xs font-semibold">
                    <div className="flex justify-between items-center bg-white/5 px-4 py-3 rounded-xl border border-white/10">
                      <div>
                        <span className="text-[8px] text-slate-500 uppercase tracking-widest block">Nomor Virtual Account {paymentMethods.find(m => m.id === selectedMethod)?.name}</span>
                        <span className="text-sm font-mono font-black text-blue-500 block mt-0.5">{adminPaymentSettings?.noRekening || paymentMethods.find(m => m.id === selectedMethod)?.value}</span>
                      </div>
                      <button 
                        onClick={() => { navigator.clipboard.writeText(adminPaymentSettings?.noRekening || paymentMethods.find(m => m.id === selectedMethod)?.value || '');alert('Nomor VA berhasil disalin!');}}                        className="px-3 py-1.5 bg-slate-850 hover:bg-slate-800 text-[10px] font-black text-slate-300 rounded-lg border border-slate-800 cursor-pointer transition-colors"
                      >
                        Salin No
                      </button>
                    </div>
                    <div className="space-y-1.5 pt-1">
                      ...
                      <ol className="list-decimal pl-4 text-[10.5px] text-slate-400 font-semibold space-y-1">
                        ...
                        <li>
                          Pilih menu <b>Transfer</b> &gt; <b>Transfer Ke Virtual Account</b>.
                        </li>
        ...
        <div className="space-y-6">
          ...
          {isWaitingConfirm && (
            <div className="bg-card-aplikasi border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4 text-center">
              ...
              <div className="bg-background-aplikasi p-3.5 rounded-2xl border border-slate-850 text-left space-y-1">
          ...
          {/* Upload Proof Form */}
          {!isPaid && !isWaitingConfirm && (
            <div className="bg-card-aplikasi border border-slate-800 rounded-3xl p-6 shadow-xl space-y-6">
              <h4 className="text-xs font-black text-slate-200 uppercase tracking-widest border-b border-slate-800 pb-2.5">
              ...
              <div className="space-y-4">
                ...
                {uploadLog && (
                  <div className="bg-background-aplikasi p-3 rounded-xl border border-slate-850 font-mono text-[9px] text-emerald-400 truncate">
                    {uploadLog}
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-xl text-xs font-black shadow-lg shadow-pink-600/15 transition-all cursor-pointer disabled:cursor-not-allowed"
                >
          ...
          <div className="bg-card-aplikasi/40 p-4 border border-slate-850 rounded-2xl flex items-start gap-3">
