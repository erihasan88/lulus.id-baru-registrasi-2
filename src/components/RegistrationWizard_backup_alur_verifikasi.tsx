...
export default function RegistrationWizard({ 
  ...
  const nextStep = () => {
    ...
    if (step < 6) {
      ...
      setTimeout(() => {
        const registrationSequence = Math.floor(1 + Math.random() * 999999)
          .toString()
          .padStart(6, '0');
        const generatedNum = `FRM-PPDB-2026-${registrationSequence}`;
        const user = `2026${registrationSequence}`;
        const password = formData.tgl_lahir ? formData.tgl_lahir.split("-").reverse().join("").slice(0, 4) + formData.tgl_lahir.slice(2, 4) : "";
        ...
        fetch('/api/registration/public/register/', {
          ...
          body: JSON.stringify({
            ...
            registration_fee: formData.tipe_kelas === 'Karyawan' ? regFeeKaryawan : regFeeReguler,
            monthly_fee: formData.tipe_kelas === 'Karyawan' ? sppKaryawan : sppReguler,
            biodata: {
              nama: formData.nama,
              nik: formData.nik,
              nisn: formData.nisn,
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl text-left text-[10px] space-y-2">
                ...
                <div className="flex justify-between font-bold text-slate-500">
                  ...
                  <span className="text-slate-800 font-black">{generatedPassword}</span>
                </div>
                <div className="flex justify-between font-bold text-slate-500">
