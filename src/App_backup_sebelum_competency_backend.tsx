...
export default function App() {
    ...
    const loadStudents = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          const hasFajar = mappedStudents.some(
            (s: Student) => s.username === 'fajar' || s.id === 'SIS-1001'
          );
          ...
          if (hasFajar) {
            ...
            setStudents(prev => {
              const fajar = prev.find(
                s => s.username === 'fajar' || s.id === 'SIS-1001'
              );
              return fajar ? [fajar, ...mappedStudents] : mappedStudents;
            });
          }
        }
      } catch (err) {
  ...
  const handleLogin = async () => {
    ...
    // 1. Admin wajib menggunakan autentikasi Django.
    // Mock login admin dinonaktifkan agar fitur admin memakai token database asli.
    if (enteredUsername === 'admin' || enteredUsername === 'admin utama' || enteredUsername === 'administrator') {
      triggerModal(
        'Login Admin Gagal',
        'Akun admin harus masuk menggunakan username dan password yang terdaftar di database Django.',
        'warning'
      );
      setIsLoggingIn(false);
      return;
  ...
  const handleRegisterSuccess = (newUsername: string, newStudent?: Student) => {
    ...
    triggerModal(
      ...
      `Daftar kesetaraan berhasil! Berkas Anda sedang menunggu verifikasi admin. Silakan coba masuk ke portal secara berkala menggunakan username: ${newUsername} dan password: ${newStudent?.password || '-'}.`, 
      'success'
    );
  };
