...
interface TranscriptNilaiProps {
  students: Student[];
  subjects: Subject[];
  loggedInUser: {
    id: string;
...
export default function TranscriptNilai({
  students,
  subjects,
  loggedInUser,
  lembagaIdentitas: propsLembagaIdentitas
}: TranscriptNilaiProps) {
  ...
  // Filter application
  const filteredTranscripts = publishedTranscripts.filter(t => {
    if (filterSemester !== 'all' && t.semester !== filterSemester) return false;
    if (filterTahunAjaran !== 'all' && t.tahunAjaran !== filterTahunAjaran) return false;
  ...
  const failingSubjects = activeTranscript 
    ? activeTranscript.subjects.filter(sub => sub.score < sub.kkm)
    : [];

  const passedCount = activeTranscript 
    ? activeTranscript.subjects.filter(sub => sub.score >= sub.kkm).length
    : 0;

  if (showPdfView && activeTranscript) {
    return (
      <PdfViewer 
