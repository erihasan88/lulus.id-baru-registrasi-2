...
class PengumpulanTugasSiswa(models.Model):
    STATUS_CHOICES = [
    ...
    tugas = models.ForeignKey(
        TugasPembelajaran,
        on_delete=models.CASCADE,
        related_name='pengumpulan_siswa'
    )
    ...
    jawaban_teks = models.TextField(
        blank=True,
        default=''
    )

    file_jawaban = models.FileField(
        upload_to='tugas/jawaban/%Y/%m/',
        blank=True,
        null=True
    )
    ...
    feedback_guru = models.TextField(
        blank=True,
        default=''
    )
    ...
    dinilai_oleh = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name='penilaian_tugas_diberikan'
    )

    dikumpulkan_pada = models.DateTimeField(
        auto_now_add=True
    )

    dinilai_pada = models.DateTimeField(
        blank=True,
        null=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['tugas', 'siswa'],
                name='unique_pengumpulan_per_tugas_siswa'
            )
        ]
        indexes = [
            models.Index(fields=['tugas', 'status']),
            models.Index(fields=['siswa', 'status']),
        ]

    def __str__(self):
        return (
            f"{self.siswa.username} - "
            f"{self.tugas.judul} - "
            f"{self.status}"
        )
...
class UjianCBTGuru(models.Model):
    JENIS_UJIAN_CHOICES = [
        ('LATIHAN', 'Latihan'),
    ...
    nama_ujian = models.CharField(
        max_length=255
    )

    jenis_ujian = models.CharField(
        max_length=30,
        choices=JENIS_UJIAN_CHOICES
    )

    soal = models.ManyToManyField(
        BankSoalGuru,
        related_name='ujian_cbt',
        blank=True
    )
    ...
    nilai_minimum = models.PositiveIntegerField(
        default=70
    )
    ...
    acak_soal = models.BooleanField(
        default=False
    )

    acak_jawaban = models.BooleanField(
        default=False
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=['rombel', 'tanggal_mulai']
            ),
            models.Index(
                fields=['mata_pelajaran', 'jenis_ujian']
            ),
        ]
...
class PengerjaanUjianCBTSiswa(models.Model):
    STATUS_CHOICES = [
    ...
    ujian = models.ForeignKey(
        UjianCBTGuru,
        on_delete=models.CASCADE,
        related_name='pengerjaan_siswa'
    )

    siswa = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='pengerjaan_ujian_cbt'
    )
    ...
    nilai_otomatis = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        blank=True,
        null=True
    ...
    nilai_esai = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        blank=True,
        null=True
    ...
    nilai_akhir = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        blank=True,
        null=True
    ...
    jumlah_benar = models.PositiveIntegerField(
        default=0
    )

    jumlah_salah = models.PositiveIntegerField(
        default=0
    )
    ...
    dinilai_oleh = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name='penilaian_cbt_diberikan'
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['ujian', 'siswa'],
                name='unique_pengerjaan_cbt_per_siswa'
            )
        ]
        indexes = [
            models.Index(fields=['ujian', 'status']),
            models.Index(fields=['siswa', 'status']),
        ]
    ...
    def __str__(self):
        return (
            f"{self.siswa.username} - "
            f"{self.ujian.nama_ujian}"
        )


class JawabanUjianCBTSiswa(models.Model):
    id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False
    ...
    pengerjaan = models.ForeignKey(
        PengerjaanUjianCBTSiswa,
        on_delete=models.CASCADE,
    ...
    soal = models.ForeignKey(
        BankSoalGuru,
        on_delete=models.CASCADE,
        related_name='jawaban_siswa'
    )

    jawaban_pilihan = models.CharField(
        max_length=10,
        blank=True,
        default=''
    )
    ...
    skor_otomatis = models.DecimalField(
        max_digits=6,
        decimal_places=2,
        blank=True,
        null=True
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['pengerjaan', 'soal'],
                name='unique_jawaban_per_soal_cbt'
            )
        ]
        indexes = [
            models.Index(fields=['pengerjaan', 'soal']),
        ]

    def __str__(self):
        return (
            f"{self.pengerjaan.siswa.username} - "
            f"{self.soal.judul}"
        )


class AgendaWajib(models.Model):
    STATUS_CHOICES = [
        ('TERJADWAL', 'Terjadwal'),
    ...
    JENIS_CHOICES = [
        ('UTS', 'UTS'),
        ('UAS', 'UAS'),
        ('CBT_TERJADWAL', 'CBT Terjadwal'),
        ('TUTORIAL_TATAP_MUKA', 'Tutorial Tatap Muka'),
        ('PRAKTIK', 'Praktik'),
        ('PEMBINAAN', 'Pembinaan'),
        ('ZOOM_WAJIB', 'Zoom Wajib'),
        ('LAINNYA', 'Lainnya'),
    ]
    ...
    nama_agenda = models.CharField(
        max_length=255
    )

    jenis_agenda = models.CharField(
        max_length=30,
    ...
    lokasi_atau_link = models.CharField(
        max_length=500,
        blank=True,
        default=''
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=['rombel', 'tanggal']
            ),
            models.Index(
                fields=['tahun_ajaran', 'status']
            ),
        ]

    def __str__(self):
        return f"{self.nama_agenda} - {self.rombel}"


class KehadiranAgenda(models.Model):
    STATUS_CHOICES = [
        ('HADIR', 'Hadir'),
        ('IZIN', 'Izin'),
        ('SAKIT', 'Sakit'),
    ...
    dicatat_oleh = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='kehadiran_agenda_dicatat'
    )

    dicatat_pada = models.DateTimeField(
        auto_now=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['agenda', 'siswa'],
                name='unique_kehadiran_siswa_per_agenda'
            )
        ]
        indexes = [
            models.Index(
                fields=['agenda', 'status']
            ),
            models.Index(
                fields=['siswa', 'status']
            ),
    ...
    def __str__(self):
        return (
            f"{self.siswa.username} - "
            f"{self.agenda.nama_agenda} - "
            f"{self.status}"
        )


class PengajuanIzinAgenda(models.Model):
    JENIS_CHOICES = [
        ('IZIN', 'Izin'),
        ('SAKIT', 'Sakit'),
    ]
    ...
    siswa = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='pengajuan_izin_agenda'
    )
    ...
    bukti = models.FileField(
        upload_to='agenda_wajib/bukti_izin/',
        null=True,
        blank=True
    )
    ...
    diverifikasi_oleh = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='pengajuan_izin_diverifikasi'
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['agenda', 'siswa'],
                name='unique_pengajuan_izin_siswa_per_agenda'
            )
        ]
        indexes = [
            models.Index(
