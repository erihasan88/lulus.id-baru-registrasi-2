...
from .models import (
    ...
    BankSoalGuru,
    UjianCBTGuru, PengerjaanUjianCBTSiswa, JawabanUjianCBTSiswa,
    AgendaWajib,
    KehadiranAgenda,
    PengajuanIzinAgenda,
)
from .serializers import (
    ...
    UjianCBTGuruSerializer, UjianCBTSiswaSerializer, PengerjaanUjianCBTSiswaSerializer,
    ...
    def patch(self, request, pk):
        ...
        if action == 'ACCEPT':
            ...
            field_mapping = {
                ...
                'status_dapodik': 'status_dapodik',
                'tanggal_terdaftar_dapodik': (
                    'tanggal_terdaftar_dapodik'
                ),
                'catatan_dapodik': 'catatan_dapodik',
                'tanggal_masuk_sekolah': (
                    'tanggal_masuk_sekolah'
                ),
            }

            allowed_dapodik_statuses = {
                'BELUM_DIATUR',
                'BELUM_DIDAFTARKAN',
                'PROSES_PENDATAAN',
                'SUDAH_TERDAFTAR',
            ...
            status_dapodik = student_data.get('status_dapodik')
            if (
                status_dapodik is not None
                and status_dapodik not in allowed_dapodik_statuses
            ):
                return Response(
                    {'detail': 'Status pendataan Dapodik tidak valid.'},
                    status=status.HTTP_400_BAD_REQUEST
                )

            for request_key, biodata_key in field_mapping.items():
                if request_key in student_data:
                    value = student_data.get(request_key)

                    if isinstance(value, str):
                        value = value.strip()

                    biodata[biodata_key] = value or ''

            if 'program' in student_data:
                registration.program_paket = (
                    str(student_data.get('program') or '').strip()
                    or registration.program_paket
                )

            if 'tipe_kelas' in student_data:
                registration.tipe_kelas = (
                    str(student_data.get('tipe_kelas') or '').strip()
                    or registration.tipe_kelas
                )
...
class AdminRegistrationPlotRombelView(views.APIView):
    ...
    def patch(self, request, pk):
        ...
        # Tanggal resmi masuk dicatat satu kali saat akun pertama kali aktif.
        # Jika sudah pernah diisi atau dikoreksi Admin, jangan ditimpa.
        if not biodata.get('tanggal_masuk_sekolah'):
            biodata['tanggal_masuk_sekolah'] = (
                timezone.localdate().isoformat()
            )

        if not biodata.get('status_dapodik'):
            biodata['status_dapodik'] = 'BELUM_DIATUR'
        ...
        for user in users:
            ...
            if current_role == 'admin':
                ...
                if role == 'admin':
                    ...
                    if teacher and registration:
                        teacher_classes = {
                            str(item).strip().lower()
                            for item in (
                                teacher.kelas_list or []
                            )
                            if str(item).strip()
                        }
...
class SiswaMateriListView(views.APIView):
    """
    Daftar materi terbit sesuai rombel siswa yang sedang login.
    """
    permission_classes = [permissions.IsAuthenticated]
...
class SiswaUjianCBTListView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request):
        ...
        queryset = (
            ...
            .prefetch_related(
                'soal',
                'pengerjaan_siswa',
                'pengerjaan_siswa__jawaban'
            )
            .filter(
                rombel_id=rombel_id,
                status__in=['TERJADWAL', 'AKTIF', 'SELESAI']
            )
        )

        return Response(
            UjianCBTSiswaSerializer(
                queryset,
                many=True,
                context={'request': request}
            ).data
...
class SiswaUjianCBTMulaiView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        ujian = get_object_or_404(
            UjianCBTGuru.objects.prefetch_related('soal'),
            pk=pk,
            rombel_id=rombel_id,
            status='AKTIF'
        )
        ...
        pengerjaan, created = (
            PengerjaanUjianCBTSiswa.objects.get_or_create(
                ujian=ujian,
                siswa=request.user,
                defaults={
        ...
        if not created and pengerjaan.status == 'SELESAI':
            return Response(
                {'detail': 'Ujian ini sudah selesai dikerjakan.'},
                status=status.HTTP_400_BAD_REQUEST
            )

        return Response(
            PengerjaanUjianCBTSiswaSerializer(
                pengerjaan,
                context={'request': request}
            ).data,
            status=(
...
class SiswaUjianCBTSubmitView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        if not profile or profile.role != 'siswa':
            return Response(
                {
                    'detail':
                    'Endpoint ini hanya dapat diakses siswa.'
                },
                status=status.HTTP_403_FORBIDDEN
            )

        pengerjaan = get_object_or_404(
            PengerjaanUjianCBTSiswa.objects
            .select_related(
                'ujian',
                'ujian__mata_pelajaran',
                'ujian__rombel',
                'ujian__rombel__tahun_ajaran',
                'siswa'
            )
            .prefetch_related('ujian__soal'),
            ujian_id=pk,
            siswa=request.user
        )
        ...
        if pengerjaan.status == 'SELESAI':
            return Response(
                {'detail': 'Ujian ini sudah selesai.'},
                status=status.HTTP_400_BAD_REQUEST
            )

        jawaban_data = request.data.get('jawaban', [])

        if not isinstance(jawaban_data, list):
            return Response(
                {
                    'jawaban':
                    'Format jawaban harus berupa daftar.'
                },
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        soal_map = {
            str(soal.id): soal
            for soal in pengerjaan.ujian.soal.all()
        }

        jawaban_map = {
            str(item.get('soal_id', '')): item
            for item in jawaban_data
        ...
        def hitung_skor_esai(jawaban, panduan, bobot):
            jawaban_bersih = normalisasi_teks(jawaban)
            panduan_bersih = normalisasi_teks(panduan)

            if not jawaban_bersih:
                return Decimal('0.00')

            if not panduan_bersih:
                return Decimal(str(bobot))

            token_jawaban = set(jawaban_bersih.split())
            token_panduan = set(panduan_bersih.split())

            kecocokan_kata = (
                len(token_jawaban & token_panduan) /
                len(token_panduan)
                if token_panduan
                else 0
            )

            kemiripan_teks = SequenceMatcher(
                None,
                jawaban_bersih,
                panduan_bersih
            ).ratio()
            ...
            panjang_minimum = min(
                1,
                len(jawaban_bersih) /
                max(20, len(panduan_bersih) * 0.35)
            )

            rasio = (
                (kecocokan_kata * 0.55) +
                (kemiripan_teks * 0.30) +
                (panjang_minimum * 0.15)
            )

            rasio = max(0, min(1, rasio))

            return Decimal(
                str(round(float(bobot) * rasio, 2))
            )

        for soal_id, soal in soal_map.items():
            item = jawaban_map.get(soal_id, {})
            bobot = int(soal.bobot or 0)

            total_bobot += bobot

            if soal.jenis_soal == 'PILIHAN_GANDA':
                pilihan = str(
                    item.get('jawaban_pilihan', '')
                ).strip().upper()

                jawaban_benar = str(
                    soal.jawaban_benar or ''
                ).strip().upper()
                ...
                skor = Decimal(
                    str(bobot if benar else 0)
                )

                total_bobot_pg += bobot
                total_skor_pg += skor
                total_skor += skor
                ...
                JawabanUjianCBTSiswa.objects.update_or_create(
                    pengerjaan=pengerjaan,
                    soal=soal,
                    defaults={
                        'jawaban_pilihan': pilihan,
                        'jawaban_esai': '',
                        'benar': benar,
                        'skor_otomatis': skor,
                        'skor_guru': None,
                ...
                jawaban_esai = str(
                    item.get('jawaban_esai', '')
                ).strip()

                skor = hitung_skor_esai(
                    jawaban_esai,
                    soal.jawaban_benar,
                    bobot
                )

                total_bobot_esai += bobot
                total_skor_esai += skor
                total_skor += skor
                ...
                JawabanUjianCBTSiswa.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'skor_guru': None,
                        'feedback_guru':
                            'Dinilai otomatis oleh sistem CBT.'
                    }
                )
        ...
        nilai_esai = (
            round(
                float(total_skor_esai) /
                total_bobot_esai * 100,
                2
            )
            if total_bobot_esai > 0
            else 0
        )
        ...
        pengerjaan.nilai_otomatis = nilai_pg
        pengerjaan.nilai_esai = nilai_esai
        pengerjaan.nilai_akhir = nilai_akhir
        pengerjaan.jumlah_benar = jumlah_benar
        pengerjaan.jumlah_salah = jumlah_salah
        pengerjaan.status = 'SELESAI'
        pengerjaan.selesai_pada = timezone.now()
        pengerjaan.save()

        ujian = pengerjaan.ujian
        tahun_ajaran = ujian.rombel.tahun_ajaran
        ...
        # Ambil hasil CBT terbaik pada mapel,
        # semester dan tahun ajaran yang sama.
        hasil_cbt = (
            PengerjaanUjianCBTSiswa.objects
            .filter(
                siswa=request.user,
                status='SELESAI',
                ujian__mata_pelajaran=ujian.mata_pelajaran,
                ujian__rombel__tahun_ajaran=tahun_ajaran,
        ...
        nilai_terbaik = max(
            [
                float(item)
                for item in hasil_cbt
                if item is not None
            ] or [nilai_akhir]
        )

        nilai_minimum = float(
            ujian.nilai_minimum or 75
        )

        status_nilai = (
            'TUNTAS'
            if nilai_terbaik >= nilai_minimum
            else 'BELUM_TUNTAS'
        )

        student_grade, created = (
            StudentGrade.objects.get_or_create(
                student=request.user,
                subject=ujian.mata_pelajaran,
                academic_year=academic_year,
                semester=semester,
                defaults={
                    'final_grade': nilai_terbaik,
                    'kkm': nilai_minimum,
                    'status': status_nilai,
                    'teacher_notes':
                        'Nilai otomatis dari Ujian Online CBT.',
                    'updated_by': None
                }
            )
        )
        ...
        if not created:
            nilai_lama = float(
                student_grade.final_grade or 0
            )

            # Remedial tidak menurunkan nilai sebelumnya.
            nilai_disimpan = max(
                nilai_lama,
                nilai_terbaik
            )

            student_grade.final_grade = nilai_disimpan
            student_grade.kkm = nilai_minimum
            student_grade.status = (
                'TUNTAS'
                if nilai_disimpan >= nilai_minimum
                else 'BELUM_TUNTAS'
            )
            student_grade.teacher_notes = (
                'Nilai terbaik otomatis dari '
                'Ujian Online CBT/remedial.'
            )
            student_grade.save()

        result = PengerjaanUjianCBTSiswaSerializer(
            pengerjaan,
            context={'request': request}
        ).data

        result['nilai_terbaik'] = nilai_terbaik
        result['status_nilai'] = status_nilai
        result['boleh_remedial'] = (
            nilai_terbaik < nilai_minimum
        )
        result['kkm'] = nilai_minimum
...
class GuruStudentCompetencyBaseView(
    GuruMateriBaseView
):
    def get_teacher_scope(self, teacher):
        teacher_mapels = {
            str(item).strip().lower()
            for item in (teacher.mapels or [])
            if str(item).strip()
        ...
        allowed_mapels = [
            item
            for item in mapel_queryset
            if item.nama.strip().lower()
            in teacher_mapels
        ]

        allowed_mapel_ids = {
            item.id for item in allowed_mapels
        }
        ...
        competency_queryset = (
            ...
            .filter(
                aktif=True,
                subject_id__in=allowed_mapel_ids
            )
            .order_by(
                'subject__nama',
                'nama_kompetensi'
        ...
        for registration in registrations:
            ...
            if rombel_nama in teacher_classes:
                allowed_students.append(
                    registration.user
                )

        return {
            'mapel_ids': allowed_mapel_ids,
            'competencies': competency_queryset,
...
class GuruStudentCompetencyListView(
    GuruStudentCompetencyBaseView
):
    @transaction.atomic
    def get(self, request):
        teacher, error_response = self.get_teacher(
            request
        )
        ...
        scope = self.get_teacher_scope(teacher)
        ...
        if new_records:
            StudentCompetency.objects.bulk_create(
                new_records,
                ignore_conflicts=True
            )

        queryset = (
            StudentCompetency.objects
            .select_related(
                'student',
            ...
            .order_by(
                'student__username',
                'competency__subject__nama',
                'competency__nama_kompetensi'
            )
        )
        ...
        student_filter = (
            request.query_params.get('siswa')
        )
        mapel_filter = (
...
class GuruStudentCompetencyUpdateView(
    GuruStudentCompetencyBaseView
):
    ...
    def patch(self, request, pk):
        ...
        nilai_baru = payload.get('nilai')

        if (
            requested_status == 'tercapai'
            and nilai_baru in {None, ''}
            and student_competency.nilai is None
        ):
            return Response(
                {
                    'nilai': [
                        'Nilai wajib diisi untuk '
                        'status tercapai.'
                    ]
                },
                status=status.HTTP_400_BAD_REQUEST
