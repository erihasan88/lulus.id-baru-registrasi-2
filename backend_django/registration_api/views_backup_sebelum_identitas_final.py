...
class LibraryBookListCreateView(views.APIView):
    ...
    def get(self, request):
        books = (
            LibraryBook.objects.all()
            if request.user.is_staff
            else LibraryBook.objects.filter(status='PUBLISHED')
        )
...
class StudentGradeListCreateView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]

    def get(self, request):
        queryset = StudentGrade.objects.select_related(
            'student', 'subject', 'updated_by'
        )
    ...
    def post(self, request):
        if not request.user.is_staff:
            return Response(
                {"detail": "Hanya admin atau guru yang dapat menginput nilai."},
                status=status.HTTP_403_FORBIDDEN
            )
...
class StudentGradeDetailView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]

    def patch(self, request, pk):
        if not request.user.is_staff:
            return Response(
                {"detail": "Hanya admin atau guru yang dapat mengubah nilai."},
                status=status.HTTP_403_FORBIDDEN
            )
